...
createInput.prototype.removeActive = function(self){
    //this.field.removeChildAt(this.field.length);
	//alert("testi");
	self.activeFlasher.destroy();
	self.activeFlasher = null;
	console.log("alert");
	$('canvas').off();
};
createInput.prototype.updateText = function(self,e){
    var newWidth = self.backgroundField.val().substring(0,self.backgroundField.val().length-(self.backgroundField.val().length-document.getElementById("userInput").selectionStart));
    ...
    if(e.which != 37 && e.which != 39) {
		...
		//alert(document.getElementById("userInput").selectionStart);
